...
const state = {
  ...
  chainId: null,
  chainName: null,
  ethersProvider: null, // this is "provider" for Ethers.js
  isConnected: false,
  w3mProvider: null, // this is "provider" from Web3Modal
  web3Modal: null
};
...
const actions = {
  ...
  async connectWeb3Modal({ commit }) {
    let w3mProvider = await state.web3Modal.connect();
    commit("setIsConnected", true);
  ...
  async ethereumListener({ commit }) {
    ...
    window.ethereum.on('accountsChanged', (accounts) => {
      if (state.isConnected) {
        ...
        commit("setEthersProvider", state.w3mProvider);
        actions.fetchActiveBalance({ commit });
      }
    });
  ...
  async fetchActiveBalance({ commit }) {
    let balance = await state.ethersProvider.getBalance(state.activeAccount);
...
const mutations = {
  ...
  async setEthersProvider(state, w3mProvider) {
